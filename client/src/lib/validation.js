...
export function validateAddressFields({ postal_code, country, mobile, phone, requireMobile = false }) {
  const errors = {}

  const postalError = validatePostalCode(postal_code, country)
  if (postalError) errors.postal_code = postalError

  const mobileError = validatePhoneE164(mobile, { required: requireMobile })
